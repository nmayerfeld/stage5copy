...
public class UDPMessageSender extends Thread implements LoggingServer {
    ...
    public void run() {
        while (!this.isInterrupted()) {
            try {
                if(this.logger == null){
                    this.logger = initializeLogging("logs/Sender-Logs",UDPMessageSender.class.getCanonicalName() + "-on-server-with-udpPort-" + this.serverUdpPort);
                }
                ...
                if (messageToSend != null) {
                    ...
                    socket.close();
                    this.logger.fine("Message sent from port "+this.serverUdpPort+":\n" + messageToSend.toString());
                }
            }
            catch (IOException e) {
